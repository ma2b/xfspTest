...
int asrLogin(const char* login_params)
{
...
{
	int			ret						=	MSP_SUCCESS;
	const char* login_params			=	"appid = 58723fb7, work_dir = ."; //登录参数,appid与msc库绑定,请勿随意改动
	/*
	* sub:             请求业务类型
	* result_type:     识别结果格式
	...
	*/
	...
	/* 用户登录 */
	ret = asrLogin(login_params); //第一个参数是用户名，第二个参数是密码，均传NULL即可，第三个参数是登录参数
	if (MSP_SUCCESS != ret)
	{
